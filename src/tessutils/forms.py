...
def google_remap_info(row):
    ...
    try:
        ...
        log.info("[%s] Could not convert to float %s", new_row['name'], row["Latitude (example 42.71167)"])
        return None
    try:
        new_row["longitude"] = float(row["Longitude (example -1.86509)"])
    except ValueError:
        log.info("Could not convert to float %s", new_row['name'], row["Longitude (example -1.86509)"])
        return None
...
def check_common(mongo_url, input_path):
    ...
    google_forms_list = list(filter(lambda x: x is not None, map(google_remap_info, google_forms_list)))
...
def check(options):
    log.info(" ====================== CROSS GOOGLE FORMS / TESSD FILE comparison ======================")
